...
def map_joystick_to_x_axis(joystick_value):
    """
    Map joystick value (-1.0 to 1.0) to x-axis range (-250 to 250) and clamp the result.
    """
    # Scale the joystick value to the desired range
    mapped_value = joystick_value * (X_AXIS_RANGE[1] - X_AXIS_RANGE[0]) / 2.0
    # Clamp the mapped value to the range [-250, 250]
    clamped_value = max(X_AXIS_RANGE[0], min(X_AXIS_RANGE[1], mapped_value))
...
try:
    while True:
        ...
        print(f"Raw: {raw_axis_value:.2f}, Filtered: {filtered_axis:.2f}, "
              f"Delta X: {delta_x:.2f}, Desired X: {desired_pose[0]:.2f}")
