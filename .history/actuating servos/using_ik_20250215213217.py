...
try:
    while True:
        ...
        # If the joystick movement is beyond the deadzone, update the desired x-axis.
        if abs(filtered_axis) > JOYSTICK_DEADZONE:
            # Map joystick input to x-axis range
            desired_x = map_joystick_to_x_axis(filtered_axis)
            desired_pose[0] = desired_x  # Update x-axis of desired pose
        else:
            desired_x = desired_pose[0]  # No change when within the deadzone.

        # For debugging: print joystick values and computed x-axis.
        print(f"Raw: {raw_axis_value:.2f}, Filtered: {filtered_axis:.2f}, "
              f"Desired X: {desired_x:.2f}")
